...
void enterState(State ns) {
    ...
    switch (ns) {
        ...
        case STATE_SET_MQTT: 
            ...
            loadMqttTopicList();
            selectedTopicIdx = 0;
            for (size_t i = 0; i < mqttTopicList.size(); i++) {
                if (mqttTopicList[i] == mqttTopicSub) {
                    selectedTopicIdx = i;
                    break;
                }
            }
        ...
        case STATE_SET_MQTT:
            if (mqttSetStep == 2) {
                if (y > 180) {
                    ...
                    if (x < 100) { // UP
                        if (selectedTopicIdx > 0) selectedTopicIdx--;
                        drawMqttTopicSelectUI();
                    } else if (x > 220) { // DOWN
                        if (selectedTopicIdx < totalItems - 1) selectedTopicIdx++;
                        drawMqttTopicSelectUI();
                    } else if (x > 110 && x < 210) { // SELECT
                        if (totalItems > 0 && selectedTopicIdx >= 0 && selectedTopicIdx < totalItems) {
                            mqttTopicSub = mqttTopicList[selectedTopicIdx];
                        }
                        mqttSetStep++;
                        updateMqttStepDisplay();
                        drawKeyboard();
                    ...
                    int clickedRow = (y - 35) / 30;
                    int targetIdx = startIndex + clickedRow;
                    if (targetIdx >= 0 && targetIdx < (int)mqttTopicList.size()) {
                        selectedTopicIdx = targetIdx;
                        drawMqttTopicSelectUI();
                    }
                    delay(150);
                }
