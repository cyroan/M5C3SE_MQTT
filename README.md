...
## 三大顯示模式與切換機制 (Display Modes & UI Interaction)

本系統支援三種即時監控畫面，並透過彈出式選單自由切換：

1. **MONITOR (MQTT 訊息模式)**：
   - 即時顯示接收到的 MQTT 訊息（包含接收時間戳、Topic 及內文 Payload）。
   - 支援右側箭頭與觸控拖曳滾動瀏覽長訊息，右下角 `UP` / `DN` 可翻閱最近 10 筆歷史訊息。
   - **動態字體加大**：Topic 包含 `RSSI`（不限大小寫）或前綴為 `PW/#`、`Advantech/#` 時，內文字體自動由 Size 2 升為 Size 3。

2. **DIAGNOSTIC (4x3 圖示診斷模式)**：
   - 採用 4x3 圖示矩陣即時呈現 12 種機構診斷結果（對應 Address 5-9，支援 Address 7 水漩/油漩雙重觸發）。
   - 無異常時中央顯示大型綠色 **ALL PASS**；故障觸發時對應圖示恢復全亮並加上**紅色邊框**。

3. **VALUES (數值物理量模式)**：
   - 上半部即時顯示 Address 0-4 之物理量（X/Y/Z 振動速度 mm/s、軸承溫度 °C、轉速 RPM），連動 AlarmX/Y/Z 警報時速度數值變紅標記。
   - 支援點擊 Speed 列切換顯示/隱藏 `[Hidden]`，偏好設定自動持久化儲存至 SD 卡 `/gui_pref.txt`。
   - 下半部條列故障文字清單，多項故障時支援右側藍色箭頭 (`^`/`v`) 觸控捲動；無異常顯示綠色「Status Normal」。

4. **觸控切換與防誤觸機制 (v1.1.34 優化)**：
   - **叫出模式選單**：在任意監控模式下點擊螢幕，即彈出橘色「SELECT DISPLAY MODE」選單（5 秒無操作自動返回原模式）。
   - **放開觸發 (wasClicked)**：選單三個模式選項均需完整點擊並放開（Click）才觸發，解決長按穿透問題。
   - **3 秒 IDLE 冷卻防護**：進入 DIAG 或 VALUES 模式後的**前 3 秒**加入保護鎖定，期間忽略全螢幕返回觸控，徹底解決切換時因手指未及時抬起而瞬間彈回選單並誤觸 MONITOR 畫面的問題；3 秒後點擊螢幕即可再次叫出模式選單。
...
### v1.1.34 (Latest)
- **觸控防誤觸與點擊邏輯優化**：
  - **Click 放開判定**：`handleTouch` 支援 `wasClicked()`，模式選單與狀態切換改由手指放開（Click）觸發，解決長按/按壓穿透問題。
  - **診斷與數值模式 3 秒 IDLE 保護**：進入 DIAG 或 VALUES 模式後加入 3 秒冷卻防護期，期間忽略全螢幕返回觸控，防止切換進入時瞬間彈回選單。
  - **選單進入短暫防抖**：進入 MODE_SELECT 選單前 300ms 忽略觸控，避免叫出選單的同一次觸控誤選按鈕。
